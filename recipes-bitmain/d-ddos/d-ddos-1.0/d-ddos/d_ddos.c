...
/**
 * exec bmminer-api and return the response
 */
char *get_bmminer_api(char *command)
{
...
void get_pools_info(pools **c_pools)
{
    ...
    char *pool_json = get_bmminer_api(GET_POOLS_CMD);
    if (pool_json != NULL)
    {
...
{
    char *pool_json = get_bmminer_api(cmd);
    cJSON *root = cJSON_Parse(pool_json);
    cJSON *status_info = cJSON_GetObjectItem(root, "STATUS");
